---
timestamp: 'Sat Oct 18 2025 18:52:17 GMT-0400 (Eastern Daylight Time)'
parent: '[[../20251018_185217.a350a8ca.md]]'
content_id: de0a15e4548713b72b9a9025a7dd894ec10349023e292c9909a46507bbba0a8e
---
...
Deno.test("AdaptiveSchedule Concept Tests", async (t) => {
  ...
  await t.step("Operational Principle: Adding, Assigning, AI Adaptation, Unassigning, Querying", async () => {
    ...
    assert(!("error" in addBlockResult), `addTimeBlock failed: ${addBlockResult.error}`);
    ...
    assert(!("error" in assignResult1), `assignAdaptiveSchedule failed: ${assignResult1.error}`);
    ...
    const existingBlocksForPrompt = await concept.adaptiveBlocks.find({ owner: userA }).toArray();
    ...
    assert(!("error" in aiResult), `requestAdaptiveScheduleAI failed: ${aiResult.error}`);
    ...
    assert(!("error" in unassignResult), `unassignBlock failed: ${unassignResult.error}`);
    ...
    assert(!("error" in getScheduleResult), `_getAdaptiveSchedule failed: ${getScheduleResult.error}`);
    ...
    assert(!("error" in getDroppedResult), `_getDroppedTask failed: ${getDroppedResult.error}`);
  ...
  await t.step("Interesting Scenario 1: Duplicate Time Block & Task Assignment", async () => {
    ...
    assert(!("error" in addBlockResult), `addTimeBlock failed unexpectedly: ${addBlockResult.error}`);
    ...
    assert(!("error" in assignResult), `assignAdaptiveSchedule failed unexpectedly: ${assignResult.error}`);
    ...
    assertEquals(duplicateAssignResult.error, `Task ${uniqueTaskId} is already assigned to the block with ID ${timeBlockId}.`);
    ...
    assert(!("error" in newAssignResult), `assignAdaptiveSchedule failed unexpectedly: ${newAssignResult.error}`);
  ...
  await t.step("Interesting Scenario 2: Invalid Inputs", async () => {
    ...
    const existingBlockId = (await concept.addTimeBlock({ owner: testUser, start: new Date(now.getTime() + 1000), end: new Date(tomorrow.getTime() + 1000) })).timeBlockId;
  ...
  await t.step("Interesting Scenario 3: LLM Response Handling", async () => {
    ...
    mockGeminiLLM.mockResponse = JSON.stringify({
      analysis: "New schedule for Charlie.",
      adaptiveBlocks: [
        {
    ...
    assert(!("error" in aiResultC), `requestAdaptiveScheduleAI failed: ${aiResultC.error}`);
  ...
  await t.step("Interesting Scenario 4: Multiple Users", async () => {
    ...
    assert(!("error" in blockDResult), `addTimeBlock for userD failed: ${blockDResult.error}`);
    ...
    assert(!("error" in assignDResult), `assignAdaptiveSchedule for userD failed: ${assignDResult.error}`);
    ...
    assert(!("error" in blockEResult), `addTimeBlock for userE failed: ${blockEResult.error}`);
    ...
    assert(!("error" in assignEResult), `assignAdaptiveSchedule for userE failed: ${assignEResult.error}`);
    ...
    assert(!("error" in getDBlocks), `_getAdaptiveSchedule for userD failed: ${getDBlocks.error}`);
    ...
    assert(!("error" in getEBlocks), `_getAdaptiveSchedule for userE failed: ${getEBlocks.error}`);
