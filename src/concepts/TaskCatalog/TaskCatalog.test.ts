...
Deno.test("TaskCatalog Concept", async (t) => {
  ...
  await t.step(
    "createTask: successfully creates a task with minimal details",
    async () => {
      ...
      assertNotEquals(
        "error" in result,
        true,
        `Expected no error, got: ${("error" in result) ? result.error : ""}`,
      );
      ...
      assertEquals(
        createdTask.timeBlockSet,
        [],
        "timeBlockSet should be initialized as empty",
      );
      assertEquals(
        createdTask.postDependence,
        [],
        "postDependence should be initialized as empty",
      );
  ...
  await t.step(
    "createTask: successfully creates a task with all optional details",
    async () => {
      ...
      assertEquals(
        createdTask.preDependence?.sort(),
        [preTask1._id, preTask2._id].sort(),
      );
      assertEquals(createdTask.note, note);
      console.log(
        `Successfully created task with optional fields: ${
          JSON.stringify(createdTask)
        }`,
      );

      // Verify postDependence on pre-tasks
      const updatedPreTask1 = await concept.tasks.findOne({
        _id: preTask1._id,
      });
      assertExists(updatedPreTask1, "Pre-task 1 should exist");
      assertExists(
        updatedPreTask1?.postDependence,
        "Pre-task 1 should have postDependence",
      );
      assertEquals(
        updatedPreTask1?.postDependence?.includes(createdTask._id),
        true,
        `Pre-task 1 should have ${createdTask._id} in its postDependence`,
      );

      const updatedPreTask2 = await concept.tasks.findOne({
        _id: preTask2._id,
      });
      assertExists(updatedPreTask2, "Pre-task 2 should exist");
      assertExists(
        updatedPreTask2?.postDependence,
        "Pre-task 2 should have postDependence",
      );
      assertEquals(
        updatedPreTask2?.postDependence?.includes(createdTask._id),
        true,
        `Pre-task 2 should have ${createdTask._id} in its postDependence`,
      );
  ...
  await t.step("getUserTasks: returns tasks for a specific owner", async () => {
    ...
    console.log(
      `Successfully retrieved tasks for ${userBob}: ${JSON.stringify(tasks)}`,
    );
  });

  await t.step(
    "getUserTasks: returns error if no tasks for owner",
    async () => {
  ...
  await t.step(
    "assignSchedule: successfully assigns a time block",
    async () => {
      ...
      assertEquals(
        ...
        0,
        "Expected empty object on success",
      );
      console.log(
        `Successfully assigned time block '${timeBlockId}' to task ${taskToSchedule._id}`,
      );

      // Verify in DB
      const updatedTask = await concept.tasks.findOne({
        _id: taskToSchedule._id,
      });
      assertExists(
        updatedTask,
        "Task should be found after schedule assignment",
      );
      assertEquals(
        updatedTask?.timeBlockSet?.includes(timeBlockId),
        true,
        "timeBlockSet should contain the new time block",
      );
  ...
  await t.step(
    "assignSchedule: returns error if task not found or owner mismatch",
    async () => {
      console.log(
      ...
      assertEquals(
        "error" in result,
        true,
        "Expected an error for owner mismatch",
      );
      assertEquals(
        result.error,
        `Task with ID ${taskToSchedule._id} not found or not owned by ${userBob}`,
      );
  ...
  await t.step(
    "assignSchedule: returns error if time block already assigned",
    async () => {
      ...
      assertEquals(
        "error" in result,
        true,
        "Expected an error for duplicate time block",
      );
      assertEquals(
        result.error,
        `Time block ${timeBlockId} already assigned to task ${taskToSchedule._id}`,
      );
      console.log(
        "Correctly returned error for duplicate time block assignment.",
      );
    },
  );

  await t.step(
    "deleteSchedule: successfully removes a time block",
    async () => {
      ...
      console.log(
        `Successfully deleted time block '${timeBlockId}' from task ${taskToSchedule._id}`,
      );
      ...
      assertEquals(
        updatedTask?.timeBlockSet?.includes(timeBlockId),
        false,
        "timeBlockSet should no longer contain the time block",
      );
      console.log("Verified task's timeBlockSet in database.");
    },
  );
  ...
  await t.step(
    "deleteSchedule: returns error if task not found or owner mismatch",
    async () => {
      console.log(
  ...
  await t.step(
    "deleteSchedule: returns error if time block not found in task",
    async () => {
      ...
      assertEquals(
        "error" in result,
        true,
        "Expected an error for non-existent time block",
      );
      assertEquals(
        result.error,
        `Time block ${nonExistentTimeBlock} not found in task ${taskToSchedule._id}'s schedules`,
      );
  ...
  await t.step("updateTaskName: successfully updates task name", async () => {
    ...
    console.log(
      `Successfully updated task ${taskToUpdate._id} name to '${newName}'`,
    );
  ...
  await t.step(
    "updateTaskCategory: successfully updates task category",
    async () => {
  ...
  await t.step(
    "updateTaskDeadline: successfully updates task deadline",
    async () => {
  ...
  await t.step(
    "updateTask: returns error if task not found or owner mismatch",
    async () => {
      console.log(
      ...
      assertEquals(
        result.error,
        `Task with ID ${taskToUpdate._id} not found or not owned by ${userBob}`,
      );
  ...
  await t.step(
    "addPreDependence: successfully adds a pre-dependence",
    async () => {
      ...
      console.log(
        `Successfully added ${preDependentTask._id} as pre-dependence to ${taskWithPreDep._id}`,
      );

      // Verify taskWithPreDep's preDependence
      const updatedTask = await concept.tasks.findOne({
        _id: taskWithPreDep._id,
      });
      assertExists(updatedTask, `Task ${taskWithPreDep._id} should exist`);
      assertExists(
        updatedTask?.preDependence,
        `Task ${taskWithPreDep._id} should have preDependence`,
      );
      assertEquals(
        updatedTask?.preDependence?.includes(preDependentTask._id),
        true,
        `Task ${taskWithPreDep._id} should have ${preDependentTask._id} in preDependence`,
      );

      // Verify preDependentTask's postDependence
      const updatedPreDependentTask = await concept.tasks.findOne({
        _id: preDependentTask._id,
      });
      assertExists(
        updatedPreDependentTask,
        `Pre-dependent task ${preDependentTask._id} should exist`,
      );
      assertExists(
        updatedPreDependentTask?.postDependence,
        `Pre-dependent task ${preDependentTask._id} should have postDependence`,
      );
      assertEquals(
        updatedPreDependentTask?.postDependence?.includes(taskWithPreDep._id),
        true,
        `Pre-dependent task ${preDependentTask._id} should have ${taskWithPreDep._id} in postDependence`,
      );
      console.log(
        "Verified preDependence and postDependence updates in database.",
      );
    },
  );

  await t.step(
    "addPreDependence: returns error if main task not found or owner mismatch",
    async () => {
      console.log(
      ...
      assertEquals(
        result.error,
        `Task with ID ${taskWithPreDep._id} not found or not owned by ${userBob}`,
      );
      console.log(
        "Correctly returned error for owner mismatch when adding pre-dependence.",
      );
    },
  );

  await t.step(
    "addPreDependence: returns error if newPreDependence task does not exist",
    async () => {
      console.log(
      ...
      assertEquals(
        "error" in result,
        true,
        "Expected an error for non-existent pre-dependence task",
      );
      assertEquals(
        result.error,
        `Pre-dependence task with ID ${nonExistentPreDep} does not exist`,
      );
      console.log(
        "Correctly returned error for non-existent pre-dependence task.",
      );
    },
  );

  await t.step(
    "removePreDependence: successfully removes a pre-dependence",
    async () => {
      ...
      console.log(
        `Successfully removed ${preDependentTask._id} from pre-dependence of ${taskWithPreDep._id}`,
      );
      ...
      assertEquals(
        updatedTask?.preDependence?.includes(preDependentTask._id),
        false,
        `Task ${taskWithPreDep._id} should NOT have ${preDependentTask._id} in preDependence`,
      );
      ...
      assertEquals(
        updatedPreDependentTask?.postDependence?.includes(taskWithPreDep._id),
        false,
        `Pre-dependent task ${preDependentTask._id} should NOT have ${taskWithPreDep._id} in postDependence`,
      );
      console.log(
        "Verified removal of preDependence and postDependence updates in database.",
      );
    },
  );

  await t.step(
    "removePreDependence: returns error if main task not found or owner mismatch",
    async () => {
      console.log(
      ...
      console.log(
        "Correctly returned error for owner mismatch when removing pre-dependence.",
      );
    },
  );

  await t.step(
    "removePreDependence: returns error if oldPreDependence not found in task",
    async () => {
      console.log(
      ...
      assertEquals(
        "error" in result,
        true,
        "Expected an error for non-existent pre-dependence in task's list",
      );
      assertEquals(
        result.error,
        `Pre-dependence task ${nonExistentPreDep} not found in task ${taskWithPreDep._id}'s preDependence`,
      );
      console.log(
        "Correctly returned error for non-existent pre-dependence in task's list.",
      );
    },
  );
  ...
  await t.step(
    "deleteTask: successfully deletes a task with no post-dependencies",
    async () => {
      ...
      // Verify in DB
      const deletedTask = await concept.tasks.findOne({
  ...
  await t.step(
    "deleteTask: returns error if task has post-dependencies",
    async () => {
      ...
      console.log(
        `Established ${mainTask._id} as pre-dependence for ${dependentTask._id}`,
      );

      // Use userAlice as the owner for deletion, matching creation
      const deleteResMainTask = await concept.deleteTask({
        owner: userAlice,
        taskId: mainTask._id,
      });
      assertEquals(
        "error" in deleteResMainTask,
        true,
        "Expected an error because task has post-dependencies",
      );
      assertEquals(
      ...
      console.log(
        "Correctly returned error for deleting task with post-dependencies.",
      );
  ...
  await t.step(
    "deleteTask: returns error if task not found or owner mismatch",
    async () => {
      console.log(
      ...
      assertEquals(
        result.error,
        `Task with ID ${nonExistentTask} not found or not owned by ${userAlice}`,
      );
      ...
      const resultOwnerMismatch = await concept.deleteTask({
        owner: userBob,
        taskId: dependentTask._id,
      });
      assertEquals(
        "error" in resultOwnerMismatch,
        true,
        "Expected an error for owner mismatch",
      );
      assertEquals(
        resultOwnerMismatch.error,
        `Task with ID ${dependentTask._id} not found or not owned by ${userBob}`,
      );
      console.log(
        "Correctly returned error for owner mismatch during deletion.",
      );
    },
  );

  await t.step(
    "Principle Trace: Demonstrate full lifecycle of tasks as per principle",
    async () => {
      console.log("\n=== Principle Trace ===");
      ...
      assertNotEquals(
        "error" in res1,
        true,
        `Failed to create task 1: ${("error" in res1) ? res1.error : ""}`,
      );
      ...
      assertNotEquals(
        "error" in updateCatRes,
        true,
        `Failed to update category: ${
          ("error" in updateCatRes) ? updateCatRes.error : ""
        }`,
      );
      ...
      assertNotEquals(
        "error" in assignRes1,
        true,
        `Failed to assign schedule 1: ${
          ("error" in assignRes1) ? assignRes1.error : ""
        }`,
      );
      const task1AfterSchedule1 = await concept.tasks.findOne({
        _id: task1._id,
      });
      assertEquals(
        task1AfterSchedule1?.timeBlockSet?.includes(timeBlockId1),
        true,
      );
      console.log(`   Task 1 scheduled for ${timeBlockId1}`);

      console.log(
        `4. Assign another schedule '${timeBlockId2}' to '${task1Name}'`,
      );
      ...
      assertNotEquals(
        "error" in assignRes2,
        true,
        `Failed to assign schedule 2: ${
          ("error" in assignRes2) ? assignRes2.error : ""
        }`,
      );
      const task1AfterSchedule2 = await concept.tasks.findOne({
        _id: task1._id,
      });
      assertEquals(
        task1AfterSchedule2?.timeBlockSet?.includes(timeBlockId2),
        true,
      );
      ...
      assertNotEquals(
        "error" in res2,
        true,
        `Failed to create task 2: ${("error" in res2) ? res2.error : ""}`,
      );
      ...
      assertNotEquals(
        "error" in addDepRes,
        true,
        `Failed to add dependence: ${
          ("error" in addDepRes) ? addDepRes.error : ""
        }`,
      );
      ...
      console.log(
        `7. Attempt to delete Task 1 (should fail due to Task 2's dependence)`,
      );
      const deleteRes1Fail = await concept.deleteTask({
        owner: principleUser,
        taskId: task1._id,
      });
      assertEquals(
        "error" in deleteRes1Fail,
        true,
        "Expected error on deleting task with post-dependence",
      );
      assertEquals(
        deleteRes1Fail.error,
        `Task ${task1._id} cannot be deleted because it has dependent tasks (${task2._id})`,
      );
      ...
      assertNotEquals(
        "error" in removeDepRes,
        true,
        `Failed to remove dependence: ${
          ("error" in removeDepRes) ? removeDepRes.error : ""
        }`,
      );
      const task2AfterRemoveDep = await concept.tasks.findOne({
        _id: task2._id,
      });
      const task1AfterRemoveDep = await concept.tasks.findOne({
        _id: task1._id,
      });
      assertEquals(
        task2AfterRemoveDep?.preDependence?.includes(task1._id),
        false,
      );
      assertEquals(
        task1AfterRemoveDep?.postDependence?.includes(task2._id),
        false,
      );
      ...
      assertNotEquals(
        "error" in deleteScheduleRes,
        true,
        `Failed to delete schedule: ${
          ("error" in deleteScheduleRes) ? deleteScheduleRes.error : ""
        }`,
      );
      const task1AfterDeleteSchedule = await concept.tasks.findOne({
        _id: task1._id,
      });
      assertEquals(
        task1AfterDeleteSchedule?.timeBlockSet?.includes(timeBlockId1),
        false,
      );
      console.log(`   Schedule '${timeBlockId1}' removed from Task 1.`);

      console.log(`10. Delete Task 1 (should succeed now)`);
      const deleteRes1Success = await concept.deleteTask({
        owner: principleUser,
        taskId: task1._id,
      });
      assertNotEquals(
        "error" in deleteRes1Success,
        true,
        `Failed to delete task 1: ${
          ("error" in deleteRes1Success) ? deleteRes1Success.error : ""
        }`,
      );
      ...
      const deleteRes2Success = await concept.deleteTask({
        owner: principleUser,
        taskId: task2._id,
      });
      assertNotEquals(
        "error" in deleteRes2Success,
        true,
        `Failed to delete task 2: ${
          ("error" in deleteRes2Success) ? deleteRes2Success.error : ""
        }`,
      );
