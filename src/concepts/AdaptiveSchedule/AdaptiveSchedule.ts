...
export default class AdaptiveScheduleConcept {
  ...
  async _getAdaptiveSchedule(
    ...
    try {
      ...
      console.error(
        `Error in _getAdaptiveSchedule: ${(error as Error).message}`,
      );
      return {
        error: `Failed to retrieve adaptive schedule: ${
  ...
  async _getDroppedTask(
    ...
    try {
      ...
      return {
        error: `Failed to retrieve dropped tasks: ${(error as Error).message}`,
      };
    }
  }
    ...
    try {
      ...
      if (existingBlock) {
        ...
        if (
          ...
          return {
            error:
              `Failed to update existing block with task ${taskId}. Block found but not modified.`,
          };
        }
      ...
      console.error(
        `Error in assignAdaptiveSchedule: ${(error as Error).message}`,
      );
      return {
        error: `Failed to assign adaptive schedule: ${
          (error as Error).message
        }`,
      };
    }
    ...
    try {
      ...
      for (const block of llmResponse.adaptiveBlocks) {
        ...
        if (isNaN(start.getTime()) || isNaN(end.getTime())) {
          console.warn(
            `Warning: LLM proposed invalid date format for block (start: ${block.start}, end: ${block.end}). Skipping.`,
          );
        ...
        for (const taskId of block.taskIds) {
          ...
          if ("error" in assignResult) {
            console.warn(
              `Warning: Could not assign task ${taskId} from LLM proposal to block (${block.start}-${block.end}): ${assignResult.error}`,
            );
      ...
      for (const dropped of llmResponse.droppedTasks) {
        ...
        try {
          ...
          console.warn(
            `Warning: Could not add dropped task ${dropped.taskId}: ${
      ...
      const finalAdaptiveBlocks = "adaptiveBlockTable" in getBlocksResult
        ? getBlocksResult.adaptiveBlockTable
        : [];
      const finalDroppedTasks = "droppedTaskSet" in getDroppedTasksResult
        ? getDroppedTasksResult.droppedTaskSet
        : [];
      ...
      console.error(
        `Error in requestAdaptiveScheduleAI: ${(error as Error).message}`,
      );
      return {
        error: `Failed to request adaptive schedule from AI: ${
          (error as Error).message
        }`,
      };
    }
  ...
  async unassignBlock(
    ...
    try {
      ...
      if (updateResult.modifiedCount === 0) {
        ...
        return {
          error:
            `Failed to unassign task ${taskId} from block ${timeBlockId}. No modification occurred.`,
        };
      }
