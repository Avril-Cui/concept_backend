 ...
 */
async function logCurrentAdaptiveSchedule(
  concept: AdaptiveScheduleConcept,
  owner: ID,
  description: string,
) {
  console.log(
 ...
 */
async function logCurrentDroppedTasks(
  concept: AdaptiveScheduleConcept,
  owner: ID,
  description: string,
) {
...
Deno.test("AdaptiveSchedule Concept Tests", async (t) => {
  ...
  const concept = new AdaptiveScheduleConcept(
    db,
    { apiKey: "dummy-api-key" } as GeminiConfig,
  );
  ...
  await t.step(
    ...
    async () => {
      ...
      console.log(
        `  Action: requestAdaptiveScheduleAI for user ${userA} with contexted_prompt`,
      );
      const aiResult = await concept.requestAdaptiveScheduleAI({
        owner: userA,
      ...
      if ("error" in finalAdaptiveScheduleResult) {
        console.log(
          `  Error retrieving final adaptive schedule: ${finalAdaptiveScheduleResult.error}`,
        );
      } else {
        console.log(
          adaptiveBlocksToString(
            finalAdaptiveScheduleResult.adaptiveBlockTable,
          ),
        );
      }
      ...
      console.log("\nFinal Dropped Tasks:");
      const finalDroppedTasksResult = await concept._getDroppedTask({
        owner: userA,
      });
      if ("error" in finalDroppedTasksResult) {
        console.log(
          `  Error retrieving final dropped tasks: ${finalDroppedTasksResult.error}`,
        );
      } else {
  ...
  await t.step(
    ...
    async () => {
      ...
      await logCurrentAdaptiveSchedule(
        concept,
        userA,
        "after adding unique time block",
      );
      ...
      assert(
        "error" in duplicateAddBlockResult,
        "Should return an error for duplicate time block",
      );
      console.log(`  Output: ${JSON.stringify(duplicateAddBlockResult)}`);
      assertEquals(
        duplicateAddBlockResult.error,
        "An adaptive time block with this owner, start, and end already exists.",
      );
      await logCurrentAdaptiveSchedule(
        concept,
        userA,
        "after attempting to add duplicate time block",
      );
      ...
      if ("error" in assignResult) {
        throw new Error(
          `assignAdaptiveSchedule failed unexpectedly on first assignment: ${assignResult.error}`,
        );
      }
      ...
      await logCurrentAdaptiveSchedule(
        concept,
        userA,
        "after assigning first task to time block",
      );
      ...
      assert(
        "error" in duplicateAssignResult,
        "Should return an error for assigning duplicate task to same block",
      );
      console.log(`  Output: ${JSON.stringify(duplicateAssignResult)}`);
      // Need to fetch the block again to get its current _id
      const currentBlock = await concept.adaptiveBlocks.findOne({
        owner: userA,
        start,
        end,
      });
      ...
      assertEquals(
        duplicateAssignResult.error,
        `Task ${uniqueTaskId} is already assigned to the block with ID ${currentBlock._id}.`,
      );
      await logCurrentAdaptiveSchedule(
        concept,
        userA,
        "after attempting to assign duplicate task",
      );
      ...
      if ("error" in newAssignResult) {
        throw new Error(
          `assignAdaptiveSchedule failed unexpectedly on second assignment: ${newAssignResult.error}`,
        );
      }
      assertEquals(
        newAssignResult.timeBlockId,
        timeBlockId,
        "Should assign to the existing block",
      );
      console.log(`  Output: timeBlockId = ${newAssignResult.timeBlockId}`);

      const verifyBlock = await concept.adaptiveBlocks.findOne({
        _id: timeBlockId,
      });
      assert(
        verifyBlock?.taskIdSet.includes(uniqueTaskId),
        "First task should still be in the block",
      );
      assert(
        verifyBlock?.taskIdSet.includes(anotherTaskId),
        "Second task should be in the block",
      );
      assertEquals(
        verifyBlock?.taskIdSet.length,
        2,
        "Block should have two tasks",
      );
      await logCurrentAdaptiveSchedule(
        concept,
        userA,
        "after assigning another task to the same time block",
      );
    },
  );
  ...
  await t.step("Interesting Scenario 2: Invalid Inputs", async () => {
    ...
    assert(
      ...
      "Should error when start >= end for assignment",
    );
    console.log(`  Output: ${JSON.stringify(invalidAssignResult1)}`);
    assertEquals(
      invalidAssignResult1.error,
      "'start' TimeStamp must be before 'end' TimeStamp.",
    );
    ...
    console.log(
      `  Action: unassignBlock from non-existent block ${nonExistentBlockId}`,
    );
    const unassignNonExistentResult = await concept.unassignBlock({
      owner: testUser,
      taskId: testTaskId,
      timeBlockId: nonExistentBlockId,
    });
    assert(
      "error" in unassignNonExistentResult,
      "Should error when unassigning from non-existent block",
    );
    console.log(`  Output: ${JSON.stringify(unassignNonExistentResult)}`);
    assertEquals(
      unassignNonExistentResult.error,
      `Adaptive block with ID ${nonExistentBlockId} not found for owner ${testUser}.`,
    );
    ...
    console.log(
      `  Action: unassignBlock with non-existent task from block ${existingBlockId}`,
    );
    const unassignNonExistentTaskResult = await concept.unassignBlock({
      owner: testUser,
      taskId: testTaskId,
      timeBlockId: existingBlockId,
    });
    assert(
      "error" in unassignNonExistentTaskResult,
      "Should error when unassigning non-existent task from block",
    );
    console.log(`  Output: ${JSON.stringify(unassignNonExistentTaskResult)}`);
    assertEquals(
      unassignNonExistentTaskResult.error,
  ...
  await t.step("Interesting Scenario 3: LLM Response Handling", async () => {
    ...
    console.log(
      `  Action: requestAdaptiveScheduleAI for user ${userC} (new schedule)`,
    );
    const aiResultC = await concept.requestAdaptiveScheduleAI({
      owner: userC,
      contexted_prompt: contextedPromptC,
    });
    if ("error" in aiResultC) {
      throw new Error(`requestAdaptiveScheduleAI failed: ${aiResultC.error}`);
    }
    assertEquals(
      aiResultC.adaptiveBlockTable.length,
      2,
      "Should have 2 adaptive blocks for Charlie",
    );
    assertEquals(
      aiResultC.droppedTaskSet.length,
      1,
      "Should have 1 dropped task for Charlie",
    );
    assert(
      aiResultC.adaptiveBlockTable.some((b) => b.taskIdSet.includes(taskC1)),
      "TaskC1 should be scheduled",
    );
    assert(
      aiResultC.adaptiveBlockTable.some((b) => b.taskIdSet.includes(taskC2)),
      "TaskC2 should be scheduled",
    );
    assert(
      aiResultC.droppedTaskSet.some((dt) =>
        dt.taskId === taskC3 && dt.reason === llmDroppedTaskC3Reason
      ),
      "TaskC3 should be dropped with reason",
    );
    console.log(
      `  Output: Adaptive Blocks: ${aiResultC.adaptiveBlockTable.length} entries, Dropped Tasks: ${aiResultC.droppedTaskSet.length} entries`,
    );
    await logCurrentAdaptiveSchedule(
      concept,
      userC,
      "after AI scheduling for Charlie",
    );
    await logCurrentDroppedTasks(
      concept,
      userC,
      "after AI dropping tasks for Charlie",
    );

    // LLM returns malformed JSON
    mockGeminiLLM.mockResponse = "this is not json";
    console.log(
      `  Action: requestAdaptiveScheduleAI for user ${userC} (malformed JSON)`,
    );
    const malformedResult = await concept.requestAdaptiveScheduleAI({
      owner: userC,
      contexted_prompt: "dummy prompt",
    });
    assert(
      "error" in malformedResult,
      "Should error for malformed LLM response",
    );
    assert(
      malformedResult.error.includes("LLM response was not valid JSON"),
    ...
    mockGeminiLLM.mockResponse = JSON.stringify({
      ...
      adaptiveBlocks: [],
    }); // Missing droppedTasks
    console.log(
      `  Action: requestAdaptiveScheduleAI for user ${userC} (missing 'droppedTasks' array)`,
    );
    const missingFieldResult1 = await concept.requestAdaptiveScheduleAI({
      owner: userC,
      contexted_prompt: "dummy prompt",
    });
    assert(
      "error" in missingFieldResult1,
      "Should error for missing droppedTasks field",
    );
    assert(
      missingFieldResult1.error.includes(
        "LLM response missing 'droppedTasks' array.",
      ),
      "Error message should indicate missing 'droppedTasks'",
    );
    ...
    mockGeminiLLM.mockResponse = JSON.stringify({
      analysis: "Partial response",
      droppedTasks: [],
    }); // Missing adaptiveBlocks
    console.log(
      `  Action: requestAdaptiveScheduleAI for user ${userC} (missing 'adaptiveBlocks' array)`,
    );
    const missingFieldResult2 = await concept.requestAdaptiveScheduleAI({
      owner: userC,
      contexted_prompt: "dummy prompt",
    });
    assert(
      "error" in missingFieldResult2,
      "Should error for missing adaptiveBlocks field",
    );
    assert(
      missingFieldResult2.error.includes(
        "LLM response missing 'adaptiveBlocks' array.",
      ),
      "Error message should indicate missing 'adaptiveBlocks'",
    );
  ...
  await t.step("Interesting Scenario 4: Multiple Users", async () => {
    ...
    const blockDResult = await concept.addTimeBlock({
      owner: userD,
      start: startD,
      end: endD,
    });
    ...
    await logCurrentAdaptiveSchedule(
      concept,
      userD,
      `after adding block for ${userD}`,
    );

    console.log(
      `  Action: assignAdaptiveSchedule for user ${userD}, task ${taskD1}`,
    );
    const assignDResult = await concept.assignAdaptiveSchedule({
      owner: userD,
      taskId: taskD1,
      start: startD,
      end: endD,
    ...
    await logCurrentAdaptiveSchedule(
      concept,
      userD,
      `after assigning task for ${userD}`,
    );

    // User E actions
    console.log(`  Action: addTimeBlock for user ${userE}`);
    const blockEResult = await concept.addTimeBlock({
      owner: userE,
      start: startE,
      end: endE,
    });
    ...
    await logCurrentAdaptiveSchedule(
      concept,
      userE,
      `after adding block for ${userE}`,
    );

    console.log(
      `  Action: assignAdaptiveSchedule for user ${userE}, task ${taskE1}`,
    );
    const assignEResult = await concept.assignAdaptiveSchedule({
      owner: userE,
      taskId: taskE1,
      start: startE,
      end: endE,
    ...
    await logCurrentAdaptiveSchedule(
      concept,
      userE,
      `after assigning task for ${userE}`,
    );
    ...
    assertEquals(
      getDBlocks.adaptiveBlockTable.length,
      1,
      "UserD should have 1 block",
    );
    assert(
      getDBlocks.adaptiveBlockTable[0].taskIdSet.includes(taskD1),
      "UserD's block should contain taskD1",
    );
    assertEquals(
      getDBlocks.adaptiveBlockTable[0].owner,
      userD,
      "UserD's block should be owned by userD",
    );
    console.log(
      `  Output: UserD blocks: ${getDBlocks.adaptiveBlockTable.length} entries. Example: ${
        JSON.stringify(getDBlocks.adaptiveBlockTable[0].taskIdSet)
      }`,
    );
    ...
    assertEquals(
      getEBlocks.adaptiveBlockTable.length,
      1,
      "UserE should have 1 block",
    );
    assert(
      getEBlocks.adaptiveBlockTable[0].taskIdSet.includes(taskE1),
      "UserE's block should contain taskE1",
    );
    assertEquals(
      getEBlocks.adaptiveBlockTable[0].owner,
      userE,
      "UserE's block should be owned by userE",
    );
    console.log(
      `  Output: UserE blocks: ${getEBlocks.adaptiveBlockTable.length} entries. Example: ${
        JSON.stringify(getEBlocks.adaptiveBlockTable[0].taskIdSet)
      }`,
    );
  });
